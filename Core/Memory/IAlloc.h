...
public:
	...
	virtual void* Allocate( uint64_t size, uint8_t align, const char* file,
		const char* function, uint32_t line ) = 0;
	virtual void* Reallocate( void* ptr, uint64_t size, const char* file, const char* function,
		uint32_t line ) = 0;
...
#define DL_NEW( alloc, type, ... ) \
	new ( ( alloc ).Allocate( sizeof( type ), alignof( type ), __FILE__, __FUNCTION__, __LINE__ ) ) \
		type( __VA_ARGS__ )

#define DL_NEW_ARRAY( alloc, type, size ) \
	( ::DuckLib::Internal::Memory::NewArray<type>( ( alloc ), ( size ), __FILE__, __FUNCTION__, \
		__LINE__ ) )
